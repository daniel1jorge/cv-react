...
function MainEducation() {
  return (
    <div className="w3-container w3-card w3-white">
        ...
        <h2 className="w3-text-grey w3-padding-16"><i className="w3-margin-right w3-xxlarge w3-text-blue"></i>Certificaciones</h2>
        <AllCertificates/>
    </div>
  ); 
}
...
function AllCertificates(){
  return (
    <>
    {data.certifications.map((certificado, index) => (
      <div className="w3-container" key={index}>
        <h5 className="w3-opacity"><b>{certificado.Where}</b></h5>
        <h5 className="w3-opacity"><b>{certificado.description}</b></h5>
        <h6 className="w3-text-blue"><i className="w3-margin-right"></i>{certificado.date}</h6>
      </div>
      ))}
      <br/>
    </>
